...
int to_jpeg(const char* str_input_file, const char* str_output_file)
{
  ...
  /* Open the input file. */
  if ((input_file = fopen(str_input_file, READ_BINARY)) == NULL) {
      fprintf(stderr, "%s: can't open %s\n", progname, str_input_file);
      return 0;
  }

  /* Open the output file. */
  if ((output_file = fopen(str_output_file, WRITE_BINARY)) == NULL) {
      fprintf(stderr, "%s: can't open %s\n", progname, str_output_file);
      return 0;
  }
